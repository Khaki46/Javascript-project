...
form.addEventListener('submit', function (e) {
  e.preventDefault();
  const getNum = +document.getElementById('amount').value;
  let random = Math.floor(Math.random() * text.length);
  console.log(getNum);
  if (!isNaN(getNum) && getNum > 0 && getNum < 10) {
    let textList = text.slice(0, getNum)
      .map((item) => {
        return `<p class="result">${item}</p>`    
      })
    textContent.innerHTML = textList.join("");
  } else {
    textContent.innerHTML = `<p class="result">${text[random]}</p>`    
  }
})